...
func GetDBConnectionString() string {
	...
	dbPassword := os.Getenv("POSTGRES_PASSWORS")
	checkEnvVar(dbPassword, "POSTGRES_PASSWORS")
	...
	if len(missingEnvVars) > 0 {
		log.Fatalf("The following required environment variables are not set: %s", strings.Join(missingEnvVars, ", "))
	}
