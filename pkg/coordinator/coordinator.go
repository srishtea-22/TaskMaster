...
func (s *CoordinatorServer) executeAllScheduledTasks() {
	...
	rows, err := tx.Query(ctx, "SELECT id, common FROM tasks WHERE scheduled_at < (NOW() + INTERVAL '30 seconds') AND picked_at IS NULL ORDER BY scheduled_at FOR UPDATE SKIP LOCKED")
